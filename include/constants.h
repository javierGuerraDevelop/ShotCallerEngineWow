...
inline constexpr std::array<std::pair<int, std::string_view>, 12>
    kIdentifyingSpells{{
...
constexpr std::string_view GetClassFromIdentifyingSpells(int spell_id) {
    for (const auto& [id, class_name] : kIdentifyingSpells) {
        if (id == spell_id) return class_name;
    }
...
inline constexpr std::array<
    std::tuple<std::string_view, int, std::chrono::seconds>, 15>
    kInterruptData{{
...
constexpr int GetInterruptId(std::string_view player_class) {
    for (const auto& [class_name, interrupt_id, interrupt_cd] :
         kInterruptData) {
...
inline constexpr std::array<
    std::tuple<std::string_view, std::string_view, int, std::chrono::seconds>,
    31>
    kCrowdControlData{
        {{"Death Knight", "Blinding Sleet", 207127, std::chrono::seconds{60}},
         {"Death Knight", "Gorefiend's Grasp", 207167,
...
constexpr std::chrono::seconds GetCcCd(int id) {
    for (const auto& [class_name, spell_name, spell_id, spell_cd] :
         kCrowdControlData) {
        if (id == spell_id) {
            return spell_cd;
        }
